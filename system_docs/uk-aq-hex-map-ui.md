...
- The cache proxy uploads those JSON payloads server-side to Dropbox under `error_log/YYYY-MM-DD/` with filenames like `uk_aq_error_hex_map_html_YYYYMMDDTHHMMSSZ_<shortid>.json`; Dropbox secrets are never exposed to the browser.
